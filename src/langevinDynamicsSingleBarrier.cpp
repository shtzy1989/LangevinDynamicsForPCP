#include "langevinDynamicsSingleBarrier.h"
...
int main(int argc, char* argv[]){
    ...
    // inputParser.AddOptionalArgument("-b", "--barrier", "Barrier height", 2, {"2.0", "6.0"});
    // inputParser.AddOptionalArgument("-s0", "--initialstate", "Initial state", 1, {"0"});
    // inputParser.AddOptionalArgument("-sl", "--slopehalflength", "Slope half length", 1, {"1.0"});
    ...
    inputParser.AddOption(/* -b    */ "BARRIERHEIGHTS", "Barrier heights (two)", "2.0 6.0", INPUTPARSER::InputNumberRequirement::Equal, 2, INPUTPARSER::InputOptionType::MustAndOnce, INPUTPARSER::InputType::Parameter);
    inputParser.AddOption(/* -sl   */ "BARRIERHALFWIDTH", "Barrier half width", "1.0", INPUTPARSER::InputNumberRequirement::Equal, 1, INPUTPARSER::InputOptionType::MustAndOnce, INPUTPARSER::InputType::Parameter);
    ...
    barrierHeight[0] = inputParser.GetOptionValue("BARRIERHEIGHTS")[0].to_double();
    barrierHeight[1] = inputParser.GetOptionValue("BARRIERHEIGHTS")[1].to_double();
    double initialState = inputParser.GetOptionValue("INITIALSTATE")[0].to_double();
    double slopeHalfLength = inputParser.GetOptionValue("BARRIERHALFWIDTH")[0].to_double();
