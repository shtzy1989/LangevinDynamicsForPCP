...
namespace LANGEVINEQUATION{
    ...
    public:
        ...
        __mSIMDd m_SlopePositive[2];
        __mSIMDd m_SlopeNegative[2];
        __mSIMDd m_InterceptPositive[2];
        __mSIMDd m_InterceptNegative[2];
        __mSIMDd m_CurrentBarrierHeight[2];
        ...
        __mSIMDd m_LocalBarrierIndex;
        __mSIMDd m_LocalBarrierIndexPrevious;
        __mmask8 m_bOnEnteringDifferentLocalBarrier;
        alignas(SIMDBYTESIZE) double m_LocalBarrierIndexStored[SIMDWIDTH];
        alignas(SIMDBYTESIZE) double m_LocalBarrierIndexPreviousStored[SIMDWIDTH];
        bool m_bOnEnteringDifferentLocalBarrierStored[SIMDWIDTH];
        ...
        dataLocator::FiniteInifiniteIntegerTickArray1D<BarrierStatus> m_BarrierStatus[2];
    public:
        void Clear(){
            for(int i=0;i<SIMDWIDTH;i++){
                m_TrajX[i].clear();
            ...
            m_BarrierStatus[0].Release();
            m_BarrierStatus[1].Release();
        }
        TrajectoryData(){
            ...
            m_SlopePositive[0] = _mmSIMD_set1_pd(0.0); 
            m_SlopePositive[1] = _mmSIMD_set1_pd(0.0); 
            m_SlopeNegative[0] = _mmSIMD_set1_pd(0.0); 
            m_SlopeNegative[1] = _mmSIMD_set1_pd(0.0); 
            m_InterceptPositive[0] = _mmSIMD_set1_pd(0.0); 
            m_InterceptPositive[1] = _mmSIMD_set1_pd(0.0); 
            m_InterceptNegative[0] = _mmSIMD_set1_pd(0.0); 
            m_InterceptNegative[1] = _mmSIMD_set1_pd(0.0); 
            m_CurrentBarrierHeight[0] = _mmSIMD_set1_pd(0.0); 
            m_CurrentBarrierHeight[1] = _mmSIMD_set1_pd(0.0); 
            ...
            m_LocalBarrierIndex = _mmSIMD_set1_pd(0.0);
            m_LocalBarrierIndexPrevious = _mmSIMD_set1_pd(0.0);
            m_bOnEnteringDifferentLocalBarrier = 0;

            for(int iChannel=0;iChannel<SIMDWIDTH;iChannel++){
                m_LocalBarrierIndexStored[iChannel] = 0;
                m_LocalBarrierIndexPreviousStored[iChannel] = 0;
                m_bOnEnteringDifferentLocalBarrierStored[iChannel] = false;
            }
        };
    public:
        ...
        virtual void WriteRestartFile(StreamerWriter* writer){
            ...
            alignas(SIMDBYTESIZE) double xp[SIMDWIDTH];

            alignas(SIMDBYTESIZE) double currentState[SIMDWIDTH];
            alignas(SIMDBYTESIZE) double previousState[SIMDWIDTH];
            alignas(SIMDBYTESIZE) double slopePositive[2][SIMDWIDTH];
            alignas(SIMDBYTESIZE) double slopeNegative[2][SIMDWIDTH];
            alignas(SIMDBYTESIZE) double interceptPositive[2][SIMDWIDTH];
            alignas(SIMDBYTESIZE) double interceptNegative[2][SIMDWIDTH];
            alignas(SIMDBYTESIZE) double currentBarrierHeight[2][SIMDWIDTH];
            ...
            alignas(SIMDBYTESIZE) double localBarrierIndex[SIMDWIDTH];
            alignas(SIMDBYTESIZE) double localBarrierIndexPrevious[SIMDWIDTH];
            bool bOnEnteringDifferentLocalBarrier[SIMDWIDTH];
            ...
            _mmSIMD_store_pd(xp, m_XP);

            _mmSIMD_store_pd(currentState, m_CurrentState);
            _mmSIMD_store_pd(previousState, m_PreviousState);
            _mmSIMD_store_pd(slopePositive[0], m_SlopePositive[0]);
            _mmSIMD_store_pd(slopePositive[1], m_SlopePositive[1]);
            _mmSIMD_store_pd(slopeNegative[0], m_SlopeNegative[0]);
            _mmSIMD_store_pd(slopeNegative[1], m_SlopeNegative[1]);
            _mmSIMD_store_pd(interceptPositive[0], m_InterceptPositive[0]);
            _mmSIMD_store_pd(interceptPositive[1], m_InterceptPositive[1]);
            _mmSIMD_store_pd(interceptNegative[0], m_InterceptNegative[0]);
            _mmSIMD_store_pd(interceptNegative[1], m_InterceptNegative[1]);
            _mmSIMD_store_pd(currentBarrierHeight[0], m_CurrentBarrierHeight[0]);
            _mmSIMD_store_pd(currentBarrierHeight[1], m_CurrentBarrierHeight[1]);

            _mmSIMD_store_pd(localBarrierIndex, m_LocalBarrierIndex);
            _mmSIMD_store_pd(localBarrierIndexPrevious, m_LocalBarrierIndexPrevious);
            _mmask8_store_SIMD(bOnEnteringDifferentLocalBarrier, m_bOnEnteringDifferentLocalBarrier);
            ...
            for(int i=0;i<SIMDWIDTH;i++){
                ...
                writer->WriteDouble(slopePositive[0][i]);
                writer->WriteDouble(slopePositive[1][i]);
                writer->WriteDouble(slopeNegative[0][i]);
                writer->WriteDouble(slopeNegative[1][i]);
                writer->WriteDouble(interceptPositive[0][i]);
                writer->WriteDouble(interceptPositive[1][i]);
                writer->WriteDouble(interceptNegative[0][i]);
                writer->WriteDouble(interceptNegative[1][i]);
                writer->WriteDouble(currentBarrierHeight[0][i]);
                writer->WriteDouble(currentBarrierHeight[1][i]);
                ...
                writer->WriteDouble(localBarrierIndex[i]);
                writer->WriteDouble(localBarrierIndexPrevious[i]);
                writer->WriteInt(bOnEnteringDifferentLocalBarrier[i] ? 1 : 0);
                writer->WriteDouble(m_LocalBarrierIndexStored[i]);
                writer->WriteDouble(m_LocalBarrierIndexPreviousStored[i]);
                writer->WriteInt(m_bOnEnteringDifferentLocalBarrierStored[i] ? 1 : 0);
            ...
            m_BarrierStatus[0].Write(writer);
            m_BarrierStatus[1].Write(writer);

            writer->WriteInt(9948);
        ...
        virtual void ReadRestartFile(StreamerReader* reader){
            ...
            // for tracking the size of the restart file
            size_t sizeTraj = 0;
            size_t sizeStatus = 0;
            ...
            for(int i=0;i<SIMDWIDTH;i++){
                ...
                slopePositive[0][i] = reader->ReadDouble();
                slopePositive[1][i] = reader->ReadDouble();
                slopeNegative[0][i] = reader->ReadDouble();
                slopeNegative[1][i] = reader->ReadDouble();
                interceptPositive[0][i] = reader->ReadDouble();
                interceptPositive[1][i] = reader->ReadDouble();
                interceptNegative[0][i] = reader->ReadDouble();
                interceptNegative[1][i] = reader->ReadDouble();
                currentBarrierHeight[0][i] = reader->ReadDouble();
                currentBarrierHeight[1][i] = reader->ReadDouble();
                ...
                localBarrierIndex[i] = reader->ReadDouble();
                localBarrierIndexPrevious[i] = reader->ReadDouble();
                bOnEnteringDifferentLocalBarrier[i] = reader->ReadInt() == 1;
                m_LocalBarrierIndexStored[i] = reader->ReadDouble();
                m_LocalBarrierIndexPreviousStored[i] = reader->ReadDouble();
                m_bOnEnteringDifferentLocalBarrierStored[i] = reader->ReadInt() == 1;
            ...
            m_BarrierStatus[0].Read(reader);
            m_BarrierStatus[1].Read(reader);
            ...
            int signiture = reader->ReadInt();
            if( signiture != 9948 ){
                fprintf(stderr, "Error: wrong signiture\n");
            }
            ...
            m_XP = _mmSIMD_load_pd(xp);

            m_CurrentState = _mmSIMD_load_pd(currentState);
            m_PreviousState = _mmSIMD_load_pd(previousState);
            m_SlopePositive[0] = _mmSIMD_load_pd(slopePositive[0]);
            m_SlopePositive[1] = _mmSIMD_load_pd(slopePositive[1]);
            m_SlopeNegative[0] = _mmSIMD_load_pd(slopeNegative[0]);
            m_SlopeNegative[1] = _mmSIMD_load_pd(slopeNegative[1]);
            m_InterceptPositive[0] = _mmSIMD_load_pd(interceptPositive[0]);
            m_InterceptPositive[1] = _mmSIMD_load_pd(interceptPositive[1]);
            m_InterceptNegative[0] = _mmSIMD_load_pd(interceptNegative[0]);
            m_InterceptNegative[1] = _mmSIMD_load_pd(interceptNegative[1]);
            m_CurrentBarrierHeight[0] = _mmSIMD_load_pd(currentBarrierHeight[0]);
            m_CurrentBarrierHeight[1] = _mmSIMD_load_pd(currentBarrierHeight[1]);

            m_LocalBarrierIndex = _mmSIMD_load_pd(localBarrierIndex);
            m_LocalBarrierIndexPrevious = _mmSIMD_load_pd(localBarrierIndexPrevious);
            m_bOnEnteringDifferentLocalBarrier = _mmask8_load_SIMD(bOnEnteringDifferentLocalBarrier);
    ...
    public:
        ...
        //          barrier index, high/low
        double m_BarrierHeight0[2][2]; 
        //                    barrier index,
        double m_InitialState0;
        //                   barrier index,
        double m_SlopeHalfLength0[2];
        double m_BarrierPosition0[2];
    ...
    protected:
        ...
        //                      barrier index, low/high
        __mSIMDd m_StateBarrierHeight[2][2];
        //              barrierIndex, low/high, left/right
        __mSIMDd m_StateBarrierSlope[2][2][2];
        __mSIMDd m_StateBarrierIntercept[2][2][2];
        __mSIMDd m_SlopeHalfLength[2];
        __mSIMDd m_BarrierPosition[2];
        __mSIMDd m_SlopeBoundLow[2];
        __mSIMDd m_SlopeBoundHigh[2];
        // __mSIMDd m_x1b_x1l;
        // __mSIMDd m_x1h_x1b;
        // __mSIMDd m_x2b_x2l;
        // __mSIMDd m_x2h_x2b;
...
public:
        // HISTOGRAM::HISTOGRAM m_GaussianNumberHistogram;
        // HISTOGRAM::HISTOGRAM m_LinearNumberHistogram;
    protected:
    ...
    public:
        LangevinEquationSIMDMultiple(){
            ...
            m_BarrierHeight0[0][0] = 6.0;
            m_BarrierHeight0[0][1] = 2.0;
            m_BarrierHeight0[1][0] = 6.0;
            m_BarrierHeight0[1][1] = 2.0;
            m_InitialState0 = 0;
            m_RandomizeState0 = true;
            m_SlopeHalfLength0[0] = 1.0;
            m_SlopeHalfLength0[1] = 1.0;
            m_BarrierPosition0[0] = 2.0;
            m_BarrierPosition0[1] = 3.0;
            {
                double x1b = m_BarrierPosition0[0];
                double w1 = m_SlopeHalfLength0[0];
                double x1l = x1b - w1;
                double x1h = x1b + w1;
                double x2b = m_BarrierPosition0[1];
                double w2 = m_SlopeHalfLength0[1];
                double x2l = x2b - w2;
                double x2h = x2b + w2;

                double y1 = m_BarrierHeight0[0][0];
                double y2 = m_BarrierHeight0[1][0];
                
                m_Xc0 = (x1h * y1 * (x2b - x2l) + x2l * y2 * (x1h - x1b)) /
                        (y1 * (x2b - x2l) + y2 * (x1h - x1b));
                m_Yc0 = (x1h - m_Xc0) / (x1h - x1b) * y1;
            }
            ...
            // m_GaussianNumberHistogram.Allocate(-5, 5, 1000);
            // m_LinearNumberHistogram.Allocate(0, 1, 1000);
        };
        ~LangevinEquationSIMDMultiple(){
    ...
    public:
        virtual bool WriteRestartFile(int iTrial){
            ...
            if( writer.Open(buffer, std::ios::binary) ){
                ...
                writer.WriteDouble(m_BarrierHeight0[0][0]);
                writer.WriteDouble(m_BarrierHeight0[0][1]);
                writer.WriteDouble(m_BarrierHeight0[1][0]);
                writer.WriteDouble(m_BarrierHeight0[1][1]);
                writer.WriteDouble(m_InitialState0);
                writer.WriteDouble(m_BarrierPosition0[0]);
                writer.WriteDouble(m_BarrierPosition0[1]);
                writer.WriteDouble(m_Xc0);
        ...
        virtual bool ReadRestartFile(int iTrial){
            ...
            if( reader.Open(buffer, std::ios::binary) ){
                ...
                if( reader.ReadDouble() != m_BarrierHeight0[0][0] ){ fprintf(stderr, "Error: inconsistent barrier height0 [0][0]\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_BarrierHeight0[0][1] ){ fprintf(stderr, "Error: inconsistent barrier height0 [0][1]\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_BarrierHeight0[1][0] ){ fprintf(stderr, "Error: inconsistent barrier height0 [1][0]\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_BarrierHeight0[1][1] ){ fprintf(stderr, "Error: inconsistent barrier height0 [1][1]\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_InitialState0 ){ fprintf(stderr, "Error: inconsistent initialState0\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_BarrierPosition0[0] ){ fprintf(stderr, "Error: inconsistent barrierPosition0[0]\n"); reader.Close(); return false; }
                if( reader.ReadDouble() != m_BarrierPosition0[1] ){ fprintf(stderr, "Error: inconsistent barrierPosition0[1]\n"); reader.Close(); return false; }
                // m_Xc0 is computed later
                reader.ReadDouble();//
        ...
        virtual bool ReadRestartFileAll(int iTrial){
            StreamerReader reader;
            char buffer[256];
            sprintf(buffer, "%s_%05d_%05d.restart", 
                m_InputRestartFileName.c_str(), m_Rank, iTrial);
            if( reader.Open(buffer, std::ios::binary) ){
                ...
                int simdwitdh_file = reader.ReadInt();
                if( simdwitdh_file != SIMDWIDTH ){ fprintf(stderr, "Error: inconsistent simd width (%d)\n", simdwitdh_file); reader.Close(); return false; }
                // write settings
                ...
                m_BarrierHeight0[0][0] = reader.ReadDouble();
                m_BarrierHeight0[0][1] = reader.ReadDouble();
                m_BarrierHeight0[1][0] = reader.ReadDouble();
                m_BarrierHeight0[1][1] = reader.ReadDouble();
                m_InitialState0 = reader.ReadDouble();
                m_BarrierPosition0[0] = reader.ReadDouble();
                m_BarrierPosition0[1] = reader.ReadDouble();
                m_Xc0 = reader.ReadDouble();
        ...
        virtual void Initialize(int randomNumberSeed){
            ...
            m_UniformRandomNumberSequenceSIMD.Initialize(MAXRANDOMNUMBERBANKSIZE, m_RandomNumberSeed * 2, 0.0, 1.0);
            m_UniformRandomNumberSequence.Initialize(MAXRANDOMNUMBERBANKSIZE, m_RandomNumberSeed * 3, 0.0, 1.0);
        }
        virtual void EnergyAndForce(){
            ...
            __mSIMDd _xp = m_TrajectoryData->m_XP;

            __mmask8 _bOnNegative[2], _bOnPositive[2];

            // barrier 0
            _bOnNegative[0] = _mmask8_and_SIMD(
                _mmSIMD_cmp_pd_mask(_xp, m_BarrierPosition[0], _CMP_LT_OS),            // x < 0
                _mmSIMD_cmp_pd_mask(_xp, m_SlopeBoundLow[0], _CMP_GT_OS)); // x > low

            _bOnPositive[0] = _mmask8_and_SIMD(
                _mmSIMD_cmp_pd_mask(_xp, m_Xc, _CMP_LT_OS),            // x < high
                _mmSIMD_cmp_pd_mask(_xp, m_BarrierPosition[0], _CMP_GT_OS)); // x > 0

            // barrier 1
            _bOnNegative[1] = _mmask8_and_SIMD(
                _mmSIMD_cmp_pd_mask(_xp, m_BarrierPosition[1], _CMP_LT_OS),            // x < 0
                _mmSIMD_cmp_pd_mask(_xp, m_Xc, _CMP_GT_OS)); // x > low

            _bOnPositive[1] = _mmask8_and_SIMD(
                _mmSIMD_cmp_pd_mask(_xp, m_SlopeBoundHigh[1], _CMP_LT_OS),            // x < high
                _mmSIMD_cmp_pd_mask(_xp, m_BarrierPosition[1], _CMP_GT_OS)); // x > 0

            __mSIMDd _energy = m_Zero;
            //        barrier index, left/right
            __mSIMDd _value[2][2];
            _value[0][0] = _mmSIMD_add_pd(_mmSIMD_mul_pd(_xp, m_TrajectoryData->m_SlopeNegative[0]), m_TrajectoryData->m_InterceptNegative[0]);
            _value[0][1] = _mmSIMD_add_pd(_mmSIMD_mul_pd(_xp, m_TrajectoryData->m_SlopePositive[0]), m_TrajectoryData->m_InterceptPositive[0]);
            _value[1][0] = _mmSIMD_add_pd(_mmSIMD_mul_pd(_xp, m_TrajectoryData->m_SlopeNegative[1]), m_TrajectoryData->m_InterceptNegative[1]);
            _value[1][1] = _mmSIMD_add_pd(_mmSIMD_mul_pd(_xp, m_TrajectoryData->m_SlopePositive[1]), m_TrajectoryData->m_InterceptPositive[1]);

            _energy = _mmSIMD_mask_blend_pd(_bOnNegative[0], m_Zero, _value[0][0]);
            _energy = _mmSIMD_mask_blend_pd(_bOnPositive[0], _energy, _value[0][1]);
            _energy = _mmSIMD_mask_blend_pd(_bOnNegative[1], _energy, _value[1][0]);
            _energy = _mmSIMD_mask_blend_pd(_bOnPositive[1], _energy, _value[1][1]);

            __mSIMDd _force = m_Zero;
            _force = _mmSIMD_mask_blend_pd(_bOnNegative[0], m_Zero, m_TrajectoryData->m_SlopeNegative[0]);
            _force = _mmSIMD_mask_blend_pd(_bOnPositive[0], _force, m_TrajectoryData->m_SlopePositive[0]);
            _force = _mmSIMD_mask_blend_pd(_bOnNegative[1], _force, m_TrajectoryData->m_SlopeNegative[1]);
            _force = _mmSIMD_mask_blend_pd(_bOnPositive[1], _force, m_TrajectoryData->m_SlopePositive[1]);
            _force = _mmSIMD_sub_pd(m_Zero, _force);

            m_TrajectoryData->m_E = _energy;
            m_TrajectoryData->m_F = _force;

            m_TrajectoryData->m_Barrier = _mmSIMD_mask_blend_pd(_bOnNegative[0], m_Zero, m_TrajectoryData->m_CurrentBarrierHeight[0]);
            m_TrajectoryData->m_Barrier = _mmSIMD_mask_blend_pd(_bOnPositive[0], m_TrajectoryData->m_Barrier, m_TrajectoryData->m_CurrentBarrierHeight[0]);
            m_TrajectoryData->m_Barrier = _mmSIMD_mask_blend_pd(_bOnNegative[1], m_TrajectoryData->m_Barrier, m_TrajectoryData->m_CurrentBarrierHeight[1]);
            m_TrajectoryData->m_Barrier = _mmSIMD_mask_blend_pd(_bOnPositive[1], m_TrajectoryData->m_Barrier, m_TrajectoryData->m_CurrentBarrierHeight[1]);
        };
        ...
        virtual bool PreIntegrate(int iTrial){
            ...
            for(int i=0;i<SIMDWIDTH;i++){
                m_UserData.m_bInitialValue[i] = m_InitialState0;

                // fprintf(stderr, "%d %13.7f a\n", i, m_TrajectoryData->m_BarrierState0Instance[i]);
                // fprintf(stderr, "%d %13.7f b\n", i, m_UserData.m_bInitialValue[i]);
            }
            m_UserData.m_EquilibriumConstant = m_EquilibriumConstant;
            m_TrajectoryData->m_BarrierStatus[0].SetUserData(&m_UserData);
            m_TrajectoryData->m_BarrierStatus[1].SetUserData(&m_UserData);
        ...
        static std::tuple<double, double> GetXc(double x1b, double w1, double x2b, double w2, double y1, double y2){
            double x1l = x1b - w1;
            double x1h = x1b + w1;
            double x2l = x2b - w2;
            double x2h = x2b + w2;
            
            double xc = (x1h * y1 * (x2b - x2l) + x2l * y2 * (x1h - x1b)) /
                        (y1 * (x2b - x2l) + y2 * (x1h - x1b));
            double yc = (x1h - xc) / (x1h - x1b) * y1;
        ...
        virtual void IntegrateInitializeVariables(){
            ...
            m_StateBarrierHeight[0][0] = _mmSIMD_set1_pd(m_BarrierHeight0[0][0]);
            m_StateBarrierHeight[0][1] = _mmSIMD_set1_pd(m_BarrierHeight0[0][1]);
            m_StateBarrierHeight[1][0] = _mmSIMD_set1_pd(m_BarrierHeight0[1][0]);
            m_StateBarrierHeight[1][1] = _mmSIMD_set1_pd(m_BarrierHeight0[1][1]);

            m_SlopeHalfLength[0] = _mmSIMD_set1_pd(m_SlopeHalfLength0[0]);
            m_SlopeHalfLength[1] = _mmSIMD_set1_pd(m_SlopeHalfLength0[1]);
            m_BarrierPosition[0] = _mmSIMD_set1_pd(m_BarrierPosition0[0]);
            m_BarrierPosition[1] = _mmSIMD_set1_pd(m_BarrierPosition0[1]);

            {
                ...
                std::tie(m_Xc0, m_Yc0) = GetXc(x1b, w1, x2b, w2, y1, y2);

                m_Xc = _mmSIMD_set1_pd(m_Xc0);
                m_Yc = _mmSIMD_set1_pd(m_Yc0);
            ...
            // printmmSIMDd("barrier [0][0]", &m_StateBarrierHeight[0][0]);
            // printmmSIMDd("barrier [0][1]", &m_StateBarrierHeight[0][1]);
            // printmmSIMDd("barrier [1][0]", &m_StateBarrierHeight[1][0]);
            // printmmSIMDd("barrier [1][1]", &m_StateBarrierHeight[1][1]);
            // printmmSIMDd("slopehalf  [0]", &m_SlopeHalfLength[0]);
            // printmmSIMDd("slopehalf  [1]", &m_SlopeHalfLength[1]);
            // printmmSIMDd("barrierpos [0]", &m_BarrierPosition[0]);
            // printmmSIMDd("barrierpos [1]", &m_BarrierPosition[1]);
            // printmmSIMDd("xc            ", &m_Xc);
            // printmmSIMDd("yc            ", &m_Yc);

            m_SlopeBoundLow[0] = _mmSIMD_set1_pd(m_BarrierPosition0[0] - m_SlopeHalfLength0[0]);
            m_SlopeBoundHigh[0] = _mmSIMD_set1_pd(m_BarrierPosition0[0] + m_SlopeHalfLength0[0]);

            m_SlopeBoundLow[1] = _mmSIMD_set1_pd(m_BarrierPosition0[1] - m_SlopeHalfLength0[1]);
            m_SlopeBoundHigh[1] = _mmSIMD_set1_pd(m_BarrierPosition0[1] + m_SlopeHalfLength0[1]);

            // printmmSIMDd("slopeBoundL[0]", &m_SlopeBoundLow[0]);
            // printmmSIMDd("slopeBoundH[0]", &m_SlopeBoundHigh[0]);
            // printmmSIMDd("slopeBoundL[1]", &m_SlopeBoundLow[1]);
            // printmmSIMDd("slopeBoundH[1]", &m_SlopeBoundHigh[1]);

            __mSIMDd _xb_xl[2], _xh_xb[2];
            _xb_xl[0] = _mmSIMD_sub_pd(m_BarrierPosition[0], m_SlopeBoundLow[0]);
            _xh_xb[0] = _mmSIMD_sub_pd(m_SlopeBoundHigh[0], m_BarrierPosition[0]);
            _xb_xl[1] = _mmSIMD_sub_pd(m_BarrierPosition[1], m_SlopeBoundLow[1]);
            _xh_xb[1] = _mmSIMD_sub_pd(m_SlopeBoundHigh[1], m_BarrierPosition[1]);

            // printmmSIMDd("_xb_xl[0]     ", &_xb_xl[0]);
            // printmmSIMDd("_xh_xb[0]     ", &_xh_xb[0]);
            // printmmSIMDd("_xb_xl[1]     ", &_xb_xl[1]);
            // printmmSIMDd("_xh_xb[1]     ", &_xh_xb[1]);

            m_StateBarrierSlope[0][0][0] = _mmSIMD_div_pd(m_StateBarrierHeight[0][0], _xb_xl[0]);
            m_StateBarrierSlope[0][1][0] = _mmSIMD_div_pd(m_StateBarrierHeight[0][1], _xb_xl[0]);
            m_StateBarrierSlope[0][0][1] = _mmSIMD_div_pd(_mmSIMD_sub_pd(m_Yc, m_StateBarrierHeight[0][0]), _mmSIMD_sub_pd(m_Xc, m_BarrierPosition[0]));
            m_StateBarrierSlope[0][1][1] = _mmSIMD_div_pd(_mmSIMD_sub_pd(m_Yc, m_StateBarrierHeight[0][1]), _mmSIMD_sub_pd(m_Xc, m_BarrierPosition[0]));

            m_StateBarrierSlope[1][0][0] = _mmSIMD_div_pd(_mmSIMD_sub_pd(m_StateBarrierHeight[1][0], m_Yc), _mmSIMD_sub_pd(m_BarrierPosition[1], m_Xc));
            m_StateBarrierSlope[1][1][0] = _mmSIMD_div_pd(_mmSIMD_sub_pd(m_StateBarrierHeight[1][1], m_Yc), _mmSIMD_sub_pd(m_BarrierPosition[1], m_Xc));
            m_StateBarrierSlope[1][0][1] = _mmSIMD_sub_pd(m_Zero, _mmSIMD_div_pd(m_StateBarrierHeight[1][0], _xh_xb[1]));
            m_StateBarrierSlope[1][1][1] = _mmSIMD_sub_pd(m_Zero, _mmSIMD_div_pd(m_StateBarrierHeight[1][1], _xh_xb[1]));

            // printmmSIMDd("slope[0][0][0]", &m_StateBarrierSlope[0][0][0]);
            // printmmSIMDd("slope[0][1][0]", &m_StateBarrierSlope[0][1][0]);
            // printmmSIMDd("slope[0][0][1]", &m_StateBarrierSlope[0][0][1]);
            // printmmSIMDd("slope[0][1][1]", &m_StateBarrierSlope[0][1][1]);

            // printmmSIMDd("slope[1][0][0]", &m_StateBarrierSlope[1][0][0]);
            // printmmSIMDd("slope[1][1][0]", &m_StateBarrierSlope[1][1][0]);
            // printmmSIMDd("slope[1][0][1]", &m_StateBarrierSlope[1][0][1]);
            // printmmSIMDd("slope[1][1][1]", &m_StateBarrierSlope[1][1][1]);

            m_StateBarrierIntercept[0][0][0] = _mmSIMD_sub_pd(m_Zero, _mmSIMD_mul_pd(m_StateBarrierHeight[0][0], _mmSIMD_div_pd(m_SlopeBoundLow[0], _xb_xl[0])));
            m_StateBarrierIntercept[0][1][0] = _mmSIMD_sub_pd(m_Zero, _mmSIMD_mul_pd(m_StateBarrierHeight[0][1], _mmSIMD_div_pd(m_SlopeBoundLow[0], _xb_xl[0])));
            m_StateBarrierIntercept[0][0][1] = _mmSIMD_div_pd(
                _mmSIMD_sub_pd(_mmSIMD_mul_pd(m_Xc, m_StateBarrierHeight[0][0]), _mmSIMD_mul_pd(m_Yc, m_BarrierPosition[0])), 
                _mmSIMD_sub_pd(m_Xc, m_BarrierPosition[0]));
            m_StateBarrierIntercept[0][1][1] = _mmSIMD_div_pd(
                _mmSIMD_sub_pd(_mmSIMD_mul_pd(m_Xc, m_StateBarrierHeight[0][1]), _mmSIMD_mul_pd(m_Yc, m_BarrierPosition[0])), 
                _mmSIMD_sub_pd(m_Xc, m_BarrierPosition[0]));

            m_StateBarrierIntercept[1][0][0] = _mmSIMD_div_pd(
                _mmSIMD_sub_pd(_mmSIMD_mul_pd(m_Yc, m_BarrierPosition[1]), _mmSIMD_mul_pd(m_Xc, m_StateBarrierHeight[1][0])), 
                _mmSIMD_sub_pd(m_BarrierPosition[1], m_Xc));
            m_StateBarrierIntercept[1][1][0] = _mmSIMD_div_pd(
                _mmSIMD_sub_pd(_mmSIMD_mul_pd(m_Yc, m_BarrierPosition[1]), _mmSIMD_mul_pd(m_Xc, m_StateBarrierHeight[1][1])), 
                _mmSIMD_sub_pd(m_BarrierPosition[1], m_Xc));
            m_StateBarrierIntercept[1][0][1] = _mmSIMD_mul_pd(m_StateBarrierHeight[1][0], _mmSIMD_div_pd(m_SlopeBoundHigh[1], _xh_xb[1]));
            m_StateBarrierIntercept[1][1][1] = _mmSIMD_mul_pd(m_StateBarrierHeight[1][1], _mmSIMD_div_pd(m_SlopeBoundHigh[1], _xh_xb[1]));

            // printmmSIMDd("inter[0][0][0]", &m_StateBarrierIntercept[0][0][0]);
            // printmmSIMDd("inter[0][1][0]", &m_StateBarrierIntercept[0][1][0]);
            // printmmSIMDd("inter[0][0][1]", &m_StateBarrierIntercept[0][0][1]);
            // printmmSIMDd("inter[0][1][1]", &m_StateBarrierIntercept[0][1][1]);

            // printmmSIMDd("inter[1][0][0]", &m_StateBarrierIntercept[1][0][0]);
            // printmmSIMDd("inter[1][1][0]", &m_StateBarrierIntercept[1][1][0]);
            // printmmSIMDd("inter[1][0][1]", &m_StateBarrierIntercept[1][0][1]);
            // printmmSIMDd("inter[1][1][1]", &m_StateBarrierIntercept[1][1][1]);

            // m_x1b_x1l = _mmSIMD_div_pd(m_One, _xb_xl[0]);
            // m_x1h_x1b = _mmSIMD_div_pd(m_One, _xh_xb[0]);
            // m_x2b_x2l = _mmSIMD_div_pd(m_One, _xb_xl[1]);
            // m_x2h_x2b = _mmSIMD_div_pd(m_One, _xh_xb[1]);
        ...
        void InitializePeriodicIndices(){
            ...
            m_TrajectoryData->m_XP = _mmSIMD_sub_pd(
                m_TrajectoryData->m_X, _mmSIMD_mul_pd(m_TrajectoryData->m_PeriodicIndex, m_PeriodicLength));
            ...
            // update local barrier index
            __mmask8 _0or1 = _mmSIMD_cmp_pd_mask(m_TrajectoryData->m_XP, m_Xc, 1); // xp < xc
            m_TrajectoryData->m_LocalBarrierIndex = _mmSIMD_mask_blend_pd(_0or1, m_One, m_Zero);
            m_TrajectoryData->m_LocalBarrierIndexPrevious = m_TrajectoryData->m_LocalBarrierIndex;
            m_TrajectoryData->m_bOnEnteringDifferentLocalBarrier = 0;

            _mmSIMD_store_pd(m_TrajectoryData->m_LocalBarrierIndexStored, m_TrajectoryData->m_LocalBarrierIndex);
            _mmSIMD_store_pd(m_TrajectoryData->m_LocalBarrierIndexPreviousStored, m_TrajectoryData->m_LocalBarrierIndexPrevious);
            _mmask8_store_SIMD(m_TrajectoryData->m_bOnEnteringDifferentLocalBarrierStored, m_TrajectoryData->m_bOnEnteringDifferentLocalBarrier);
        }
        ...
        void UpdatePeriodicIndices(size_t iStep){
            ...
            // update local barrier index
            m_TrajectoryData->m_LocalBarrierIndexPrevious = m_TrajectoryData->m_LocalBarrierIndex;
            memcpy(m_TrajectoryData->m_LocalBarrierIndexPreviousStored, m_TrajectoryData->m_LocalBarrierIndexStored, sizeof(double) * SIMDWIDTH);

            __mmask8 _0or1 = _mmSIMD_cmp_pd_mask(m_TrajectoryData->m_XP, m_Xc, 1); // xp < xc
            m_TrajectoryData->m_LocalBarrierIndex = _mmSIMD_mask_blend_pd(_0or1, m_One, m_Zero);
            m_TrajectoryData->m_bOnEnteringDifferentLocalBarrier = _mmSIMD_cmp_pd_mask(
                m_TrajectoryData->m_LocalBarrierIndexPrevious, m_TrajectoryData->m_LocalBarrierIndex, _CMP_NEQ_OQ);

            _mmSIMD_store_pd(m_TrajectoryData->m_LocalBarrierIndexStored, m_TrajectoryData->m_LocalBarrierIndex);
            _mmask8_store_SIMD(m_TrajectoryData->m_bOnEnteringDifferentLocalBarrierStored, m_TrajectoryData->m_bOnEnteringDifferentLocalBarrier);

            // bool b0or1[SIMDWIDTH];
            // _mmask8_store_SIMD(b0or1, _0or1);
            // bool trans[SIMDWIDTH];
            // _mmask8_store_SIMD(trans, m_TrajectoryData->m_bOnEnteringDifferentLocalBarrier);

            // fprintf(stdout, "%10zd %23.16E %23.16E %1d %s\n", iStep, m_TrajectoryData->m_X[0], m_TrajectoryData->m_XP[0], b0or1[0] ? 0 : 1, trans[0] ? "true" : "false");
        }
        void InitializeBarrierState(){
            ...
            const int leftmost = -10000;
            const int rightmost = 10000;
            m_TrajectoryData->m_BarrierStatus[0][leftmost];
            m_TrajectoryData->m_BarrierStatus[0][rightmost];
            m_TrajectoryData->m_BarrierStatus[1][leftmost];
            m_TrajectoryData->m_BarrierStatus[1][rightmost];

#ifndef SINGLEMOTION
            // override initial state to the initial barrier
            for(int i=0;i<SIMDWIDTH;i++){
                int index = m_TrajectoryData->m_PeriodicIndexStored[i];
                int barrierIndex = m_TrajectoryData->m_LocalBarrierIndexStored[i];
                {
                    BarrierStatus* barrierStatus = static_cast<BarrierStatus*>(m_TrajectoryData->m_BarrierStatus[barrierIndex][index]);
        ...
        void UpdateBarrierState(size_t iStep){
            ...
            for(int i=0;i<SIMDWIDTH;i++){
                if( m_TrajectoryData->m_bOnEnteringDifferentLocalBarrierStored[i] ){
                    // save status of the previous barrier
                    int indexPrevious = m_TrajectoryData->m_PeriodicIndexPreviousStored[i];
                    int indexBarrierPrevious = m_TrajectoryData->m_LocalBarrierIndexPreviousStored[i];
                    BarrierStatus* barrierStatusPrevious = static_cast<BarrierStatus*>(m_TrajectoryData->m_BarrierStatus[indexBarrierPrevious][indexPrevious]);
                    ...
                    int indexBarrierCurrent = m_TrajectoryData->m_LocalBarrierIndexStored[i];
                    BarrierStatus* barrierStatusCurrent = static_cast<BarrierStatus*>(m_TrajectoryData->m_BarrierStatus[indexBarrierCurrent][indexCurrent]);
                    // previousState[i] = barrierStatusCurrent->m_State[i];
                    currentState[i] = barrierStatusCurrent->m_State[i];

                    // if( i == 0 ) printf("%13zd %13zd [%2d, %d](%d)->[%2d, %d](%d) %13.7f %13.7f\n", 
                    //     barrierStatusCurrent->m_TimeStep[i], iStep, 
                    //     indexPrevious, indexBarrierPrevious, barrierStatusPrevious->m_State[i], 
                    //     indexCurrent, indexBarrierCurrent, (int)currentState[i], 
                    //     m_TrajectoryData->m_X[i], m_TrajectoryData->m_XP[i]);
                }
        ...
        void UpdateBarrierStateEta(size_t iStep){
            ...
            if( m_bEqualProbability ){
                ...
                _mmSIMD_store_pd(currentState, m_TrajectoryData->m_CurrentState);
                // for(int i=0;i<SIMDWIDTH;i++){
                //     double value = _uniformRandomNumber[i];
                //     m_LinearNumberHistogram.AddValue(value);
                // }
                ...
                for(int i=0;i<SIMDWIDTH;i++){
                    if( m_TrajectoryData->m_bOnEnteringDifferentLocalBarrierStored[i] ){
                        ...
                        // if( i == 0 ) fprintf(stderr, "%13zd %13zd %13.7f %13.7f [%2d, %d](%d)->[%2d, %d](%d) %13.7f %d (%13.7f %13.7f) (%13.7f %13.7f)\n", 
                        //     barrierStatusCurrent->m_TimeStep[i], iStep, timeLap, transitionProbability, 
                        //     indexPrevious, indexBarrierPrevious, barrierStatusPrevious->m_State[i], 
                        //     indexCurrent, indexBarrierCurrent, (int)currentState[i], 
                        //     previousState, previousState == 1.0,
                ...
                for(int i=0;i<SIMDWIDTH;i++){
                    if( m_TrajectoryData->m_bOnEnteringDifferentLocalBarrierStored[i] ){
                        ...
                        // if( i == 0 ) fprintf(stderr"%13zd %13zd %13.7f %13.7f [%2d, %d](%d)->[%2d, %d](%d) %13.7f %d (%13.7f %13.7f)\n", 
                        //     barrierStatusCurrent->m_TimeStep[i], iStep, timeLap, transitionProbability, 
                        //     indexPrevious, indexBarrierPrevious, barrierStatusPrevious->m_State[i], 
                        //     indexCurrent, indexBarrierCurrent, (int)currentState[i], 
                        //     previousState, previousState == 1.0,
        ...
        virtual void UpdateBarrierStateEtaParamter(){
            __mmask8 _condition = _mmSIMD_cmp_pd_mask(m_TrajectoryData->m_CurrentState, m_One, _CMP_EQ_OQ);
            // __mmask8 _condition2 = _mmSIMD_cmp_pd_mask(m_TrajectoryData->m_LocalBarrierIndex, m_One, _CMP_EQ_OQ);
            m_TrajectoryData->m_CurrentBarrierHeight[0] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierHeight[0][0], m_StateBarrierHeight[0][1]);
            m_TrajectoryData->m_CurrentBarrierHeight[1] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierHeight[1][0], m_StateBarrierHeight[1][1]);
            
            m_TrajectoryData->m_SlopeNegative[0] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierSlope[0][0][0], m_StateBarrierSlope[0][1][0]);
            m_TrajectoryData->m_SlopeNegative[1] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierSlope[1][0][0], m_StateBarrierSlope[1][1][0]);
            m_TrajectoryData->m_SlopePositive[0] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierSlope[0][0][1], m_StateBarrierSlope[0][1][1]);
            m_TrajectoryData->m_SlopePositive[1] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierSlope[1][0][1], m_StateBarrierSlope[1][1][1]);

            m_TrajectoryData->m_InterceptNegative[0] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierIntercept[0][0][0], m_StateBarrierIntercept[0][1][0]);
            m_TrajectoryData->m_InterceptNegative[1] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierIntercept[1][0][0], m_StateBarrierIntercept[1][1][0]);
            m_TrajectoryData->m_InterceptPositive[0] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierIntercept[0][0][1], m_StateBarrierIntercept[0][1][1]);
            m_TrajectoryData->m_InterceptPositive[1] = _mmSIMD_mask_blend_pd(_condition, m_StateBarrierIntercept[1][0][1], m_StateBarrierIntercept[1][1][1]);
            ...
            if( m_bFirstPassageTime ){
                ...
                for(int i=0;i<SIMDWIDTH;i++){
                    ...
                    if( Math::Modulo(m_X0, m_PeriodicLength0) == m_HalfPeriodicLength0 ){
                        ...
                        //     }
                        // }
                        m_TrajectoryData->m_InitialBarrierState[i][0] = 0;
                        m_TrajectoryData->m_InitialBarrierIndex[i][0] = 0;
                        m_TrajectoryData->m_InitialBarrierState[i][1] = 0;
                        m_TrajectoryData->m_InitialBarrierIndex[i][1] = 0;
                        // debug
                        // if( i == 0 ){
        ...
        void UpdateStatistics(size_t iStep){
            ...
            if( m_bWaitingTimeDistribution ){
                ...
                */
                // just remove all previous restart data for now
                for(int i=0;i<SIMDWIDTH;i++){
                    m_TrajectoryData->m_BarrierTimeTick[i].clear();
                }
            }
            ...
            if( m_bFirstPassageTime ){
                ...
                for(int i=0;i<SIMDWIDTH;i++){
                    // from bottom to top
                    m_TrajectoryData->m_FirstPassageTime[i].clear();
                    m_TrajectoryData->m_InstantBarrierList[i].clear();
                    ...
                    if( m_TrajectoryData->m_bOnEnteringDifferentPeriodicityStored[i] && 
                        ...
                        // debug
                        // if( i==0 ){
                        //     fprintf(stdout, "pass %23.16E %13.7f %13.7f %13.7f i %3d %3d n %3d\n", 
                        //         m_TimeStep0 * iStep, ((double*)&m_TrajectoryData->m_X)[0], ((double*)&m_TrajectoryData->m_XP)[0],
            ...
            if( m_bTransitionPathTime ){
                ...
                */
                // remove all restart data for now
                for(int i=0;i<SIMDWIDTH;i++){
                    m_TrajectoryData->m_TransitionPathTime[i].clear();
                    m_TrajectoryData->m_TransitionPathTime2[i].clear();
                }
...
// printf("count %10zd %13.7f %d\n\n", m_TrajectoryData->m_InitialBarrierList[index].size(), m_TrajectoryData->m_CurrentState[index], index);
    ...
    public:
        ...
        virtual void IntegrateStepEta(size_t iStep){
            ...
            // for(int i=0;i<SIMDWIDTH;i++){
            //     double value = rf[i] / m_VarianceOfRandomForceIntegration[i];
            //     m_GaussianNumberHistogram.AddValue(value);
            // }
            UpdatePeriodicIndices(iStep);
    ...
    public:
        ...
        int initialState;
        double slopeHalfLength[2];
        double barrierPosition[2];
        double xc;
        double yc;
    ...
    public:
        LangevinEquationRunnerMPI(MPI_Comm m_World){
            ...
            periodicLength = 5.0;
            barrierHeight[0][0] = 2.0;
            barrierHeight[0][1] = 6.0;
            barrierHeight[1][0] = 2.0;
            barrierHeight[1][1] = 6.0;
            initialState = 0;
            slopeHalfLength[0] = 1.0;
            slopeHalfLength[1] = 1.0;
            barrierPosition[0] = 2.0;
            barrierPosition[1] = 3.0;
            xc = 2.5;
            yc = 2.0;
    ...
    public:
        void PrintParameter(FILE* fout = stderr){
            ...
            fprintf(fout, "# ==============================================\n");
            fprintf(fout, "# Barrier height [0][0]= %13.7f\n", barrierHeight[0][0]);
            fprintf(fout, "# Barrier height [0][1]= %13.7f\n", barrierHeight[0][1]);
            fprintf(fout, "# Barrier height [1][0]= %13.7f\n", barrierHeight[1][0]);
            fprintf(fout, "# Barrier height [1][1]= %13.7f\n", barrierHeight[1][1]);
            fprintf(fout, "# Xc                   = %13.7f\n", xc);
            fprintf(fout, "# Yc                   = %13.7f\n", yc);
            fprintf(fout, "# Barrier periodicity  = %13.7f\n", periodicLength);
            fprintf(fout, "# Slope half length[0] = %13.7f\n", slopeHalfLength[0]);
            fprintf(fout, "# Slope half length[1] = %13.7f\n", slopeHalfLength[1]);
            fprintf(fout, "# Barrier position [0] = %13.7f\n", barrierPosition[0]);
            fprintf(fout, "# Barrier position [1] = %13.7f\n", barrierPosition[1]);
        ...
        void Setup(
            ...
            double barrierHeightB0L = 1.0, double barrierHeightB0H = 1.0, 
            double barrierHeightB1L = 1.0, double barrierHeightB1H = 1.0,
            int initialState = 0, 
            double slopeHalfLengthB0 = 1.0, double slopeHalfLengthB1 = 1.0, 
            double barrierPositionB0 = 2.0, double barrierPositionB1 = 3.0,
            ...
            this->barrierHeight[0][0] = barrierHeightB0L;
            this->barrierHeight[0][1] = barrierHeightB0H;
            this->barrierHeight[1][0] = barrierHeightB1L;
            this->barrierHeight[1][1] = barrierHeightB1H;
            this->initialState = initialState;
            this->slopeHalfLength[0] = slopeHalfLengthB0;
            this->slopeHalfLength[1] = slopeHalfLengthB1;
            this->barrierPosition[0] = barrierPositionB0;
            this->barrierPosition[1] = barrierPositionB1;
        ...
        void Run(){
            ...
            std::tie(xc, yc) = LangevinEquationSIMDMultiple::GetXc(
                barrierPosition[0], slopeHalfLength[0], barrierPosition[1], slopeHalfLength[1], barrierHeight[0][0], barrierHeight[1][0]);
            ...
            LangevinEquationSIMDMultiple* leOMP = 0;
            {
                ...
                leOMP->m_BarrierHeight0[0][0] = barrierHeight[0][0];
                leOMP->m_BarrierHeight0[0][1] = barrierHeight[0][1];
                leOMP->m_BarrierHeight0[1][0] = barrierHeight[1][0];
                leOMP->m_BarrierHeight0[1][1] = barrierHeight[1][1];
                leOMP->m_SlopeHalfLength0[0] = slopeHalfLength[0];
                leOMP->m_SlopeHalfLength0[1] = slopeHalfLength[1];
                leOMP->m_BarrierPosition0[0] = barrierPosition[0];
                leOMP->m_BarrierPosition0[1] = barrierPosition[1];
            ...
            for(int i=0;i<ntrial;i++){
                ...
                // m_MPI.SumHistogramCount(&leOMP->m_LinearNumberHistogram, 1);
                // if( m_MPI.IsMasterNode() ){
                //     FILE* fout = fopen("test.hist", "w+");
                //     leOMP->m_LinearNumberHistogram.Print(fout);
                //     fclose(fout);
                // }

                // m_MPI.SumHistogramCount(&leOMP->m_GaussianNumberHistogram, 1);
                // if( m_MPI.IsMasterNode() ){
                //     FILE* fout = fopen("test2.hist", "w+");
                //     leOMP->m_GaussianNumberHistogram.Print(fout);
                //     fclose(fout);
                // }
        ...
        void Convert(){            
            ...
            fprintf(stderr, "Write msd file\n");
            {
                ...
                for(int i=0;i<interval0.size();i++){
                    fprintf(fout, "%13.7f %13.7f %13.7f", 
    ...
    public:
        ...
        bool bRemoveDriftOfMSD;
        int numberOfSubSampling;
        double fractionOfSubSampling;
    protected:
        int nrunPerNode;
        int nrun;
        int randomNumberSeedUsed;
    ...
    public:
        LangevinEquationRunnerMPIPostProcess(){
            ...
            bRemoveDriftOfMSD = false;
            numberOfSubSampling = 20;
            fractionOfSubSampling = 0.8;
        }
    public:
        void PrintParameter(FILE* fout = stderr){
            fprintf(fout, "# ==============================================\n");
            ...
            fprintf(fout, "# Sample per order     = %13d\n", samplePerOrder);
            fprintf(fout, "# ==============================================\n");
            fprintf(fout, "# Remove Drift MSD     = %13s\n", text(bRemoveDriftOfMSD).c_str());
            fprintf(fout, "# Number of Sampling   = %13d\n", numberOfSubSampling);
            fprintf(fout, "# Fraction of Sampling = %13.7f\n", fractionOfSubSampling);
            fprintf(fout, "# ==============================================\n");
            fprintf(fout, "# X0                   = %13.7f\n", x0);
            if( bRandomizeVelocity ){
                fprintf(fout, "# V0                   = %13s\n", "Random");
        ...
        void Setup(
            ...
            bool bRemoveDriftOfMSD = false, int numberOfSubSampling = 20, double fractionOfSubSampling = 0.8){
            // code
            this->outputBase = outputBase;
            this->x0 = x0;
            this->v0 = v0;
            ...
            this->bRemoveDriftOfMSD = bRemoveDriftOfMSD;
            this->numberOfSubSampling = numberOfSubSampling;
            this->fractionOfSubSampling = fractionOfSubSampling;
        }
        void Run(){
            // code
            ...
            // to address drift
            std::vector<std::vector<double> > trajectoryList;
            if( bRemoveDriftOfMSD ){
                trajectoryList.resize(ntrial * numberOfMPIThreads * SIMDWIDTH);
            }
            // for(int i=0;i<trajectoryList.size();i++){
            //     trajectoryList[i].resize(interval.size());
            // }
            ...
            for(int i=0;i<ntrial;i++){
                ...
                for(int iMPI=0;iMPI<numberOfMPIThreads;iMPI++){
                    ...
                    // fprintf(stderr, "   Post Analysis\n");
                    timer2.Start();

                    for(int iChannel=0;iChannel<SIMDWIDTH;iChannel++){
                        // to address drift
                        if( bRemoveDriftOfMSD ){
                            trajectoryList[i * numberOfMPIThreads * SIMDWIDTH + iMPI * SIMDWIDTH + iChannel] = leOMP->GetX(iChannel);
                        }
                        ...
                        if( iMPI == numberOfMPIThreads - 1 || loopTimer2.Tick(1.0) ){
                            loopTimer2.Update(iMPI, numberOfMPIThreads);
                            fprintf(stderr, "\33[F    %10d/%10d %s\n", iMPI + 1, numberOfMPIThreads, loopTimer2.GetPChar());
                        }
                    }
                    timer2.StopToEntry("analysis");
                }
            ...
            if( bRemoveDriftOfMSD ){
                // to address drift
                std::vector<double> trajectoryAverage(trajectoryList[0].size());
                for(int i=0;i<trajectoryList.size();i++){
                    #pragma omp parallel for
                    for(int j=0;j<trajectoryList[i].size();j++){
                        trajectoryAverage[j] += trajectoryList[i][j];
                    }
                }
                #pragma omp parallel for
                for(int i=0;i<trajectoryAverage.size();i++){
                    trajectoryAverage[i] /= trajectoryList.size();
                }

                std::vector<std::vector<double> > trajectoryAverageSub(numberOfSubSampling);
                for(int r=0;r<numberOfSubSampling;r++){
                    trajectoryAverageSub[r].resize(trajectoryList[0].size());

                    std::vector<unsigned int> candidate;
                    int size = double(trajectoryList.size()) * fractionOfSubSampling;
                    RANDOMNUMBER::RANDOM_VECTOR_NON_REPEAT(&candidate, size, trajectoryList.size());

                    for(int i=0;i<candidate.size();i++){
                        int ID = candidate[i];
                        #pragma omp parallel for
                        for(int j=0;j<trajectoryList[ID].size();j++){
                            trajectoryAverageSub[r][j] += trajectoryList[ID][j];
                        }
                    }
                    #pragma omp parallel for
                    for(int j=0;j<trajectoryAverageSub[r].size();j++){
                        trajectoryAverageSub[r][j] /= candidate.size();
                    }
                }

                std::vector<double> trajectoryRMSD(trajectoryAverage.size());
                for(int i=0;i<trajectoryAverage.size();i++){
                    double rmsd = 0.0;
                    #pragma omp parallel for reduction(+:rmsd)
                    for(int r=0;r<trajectoryAverageSub.size();r++){
                        rmsd += (trajectoryAverageSub[r][i] - trajectoryAverage[i]) * 
                                (trajectoryAverageSub[r][i] - trajectoryAverage[i]);
                    }
                    rmsd = sqrt(rmsd / trajectoryAverageSub.size());
                    trajectoryRMSD[i] = rmsd;
                }

                std::vector<double> trajectoryKSTest(trajectoryAverage.size());
                for(int i=0;i<trajectoryAverage.size();i++){
                    std::vector<double> value;
                    value.reserve(trajectoryList.size());
                    for(int j=0;j<trajectoryList.size();j++){
                        value.push_back(trajectoryList[j][i]);
                    }
                    double p = STATISTICS::KSTest(value);
                    trajectoryKSTest[i] = p;
                }

                text filename = outputBase + "_averageTrajectory.log";
                TextFileWriter writer;
                if( writer.Open(filename, true) ){
                    PrintParameter(writer.GetFile());
                    for(int i=0;i<trajectoryAverage.size();i++){
                        writer.Write("%24.16E %24.16E %24.16E %24.16E ", saveTimeStep * i, trajectoryAverage[i], trajectoryRMSD[i], trajectoryKSTest[i]);
                        for(int j=0;j<trajectoryAverageSub.size();j++){
                            writer.Write("%24.16E ", trajectoryAverageSub[j][i]);
                        }
                        writer.Write("\n");
                    }
                    writer.Close();
                ...
                std::vector<double> msdA;
                {
                    // 
                    std::vector<double> xbar(interval.size());
                    for(int i=0;i<interval.size();i++){
                        int lag = interval[i];
                        double value = 0.0;
                        #pragma omp parallel for reduction(+:value)
                        for(int t=0;t<trajectoryAverage.size()-lag;t++){
                            value += trajectoryAverage[t + lag] - trajectoryAverage[t];
                        }
                        value /= double(trajectoryAverage.size()-lag);
                        xbar[i] = value;
                    }

                    //
                    msdA.resize(interval.size());
                    for(int i=0;i<trajectoryList.size();i++){
                        for(int ii=0;ii<interval.size();ii++){
                            int lag = interval[ii];
                            double value = 0.0;
                            #pragma omp parallel for reduction(+:value)
                            for(int k=0;k<trajectoryAverage.size()-lag;k++){
                                double value0 = trajectoryList[i][k + lag] - trajectoryList[i][k] - xbar[ii];
                                value += value0 * value0;
                            }
                            msdA[ii] += value / double(trajectoryAverage.size()-lag);
                        }
                    }
                    #pragma omp parallel for
                    for(int j=0;j<msdA.size();j++){
                        msdA[j] /= trajectoryList.size();
                    }
                }
                filename = outputBase + "_msdNoDriftByAverage.log";
                {
                    MOLUTILITY::findNextBackUpFileName(filename.string());
                    FILE *fout = fopen(filename.c_str(), "w+");
                    PrintParameter(fout);
                    for(int t=0;t<interval.size();t++){
                        fprintf(fout, "%13.7f %13.7f\n", saveTimeStep * interval[t], msdA[t]);
                    }
                    fclose(fout);
                }

                std::vector<double> msdL;
                std::vector<double> msdL0;
                {
                    msdL.resize(interval.size());
                    msdL0.resize(interval.size());
                    for(int i=0;i<trajectoryList.size();i++){
                        #pragma omp parallel for
                        for(int j=0;j<trajectoryList[i].size();j++){
                            trajectoryList[i][j] -= trajectoryAverage[j];
                        }
                        MeanSquareDisplacementBruteForce(
                            &trajectoryList[i], &msdL0, &interval);
                        for(int j=0;j<msdL0.size();j++){
                            msdL[j] += msdL0[j];
                        }
                    }
                    #pragma omp parallel for
                    for(int j=0;j<msdL.size();j++){
                        msdL[j] /= trajectoryList.size();
                    }
                }

                filename = outputBase + "_msdNoDrift.log";
                MOLUTILITY::findNextBackUpFileName(filename.string());
                FILE *fout = fopen(filename.c_str(), "w+");
                PrintParameter(fout);
                for(int t=0;t<interval.size();t++){
                    fprintf(fout, "%13.7f %13.7f\n", saveTimeStep * interval[t], msdL[t]);
                }
                fclose(fout);
            }
    ...
    public:
        void Run(double fraction, long nset, int ncore, double eps){
            ...
            // fprintf(stderr, "Left BI %24d %24d\n", leftBound[0], leftBound[1]);
            // fprintf(stderr, "RightBI %24d %24d\n", rightBound[0], rightBound[1]);
        ...
        std::tuple<std::vector<double>, std::vector<std::vector<double> > > ParseMSDFromRestartFile(int massID, int restartID){
            ...
            for(int i=0;i<ntrial;i++){
                ...
                for(int iMPI=0;iMPI<numberOfMPIThreads;iMPI++){
                    ...
                    for(int iChannel=0;iChannel<SIMDWIDTH;iChannel++){
                        ...
                        // to address drift
                        if( bRemoveDriftOfMSD ){
                            trajectoryList[iMSD] = leOMP->GetX(iChannel);
                        }else{
                            // msd
                            MeanSquareDisplacementBruteForce(
                                &leOMP->GetX(iChannel), &msd[iMSD], &interval);
                            for(int t=0;t<interval.size();t++){
                                if( isnan(msd[iMPI][t]) || isinf(msd[iMPI][t]) ){
                                    fprintf(stderr, "Error: on node %10d, ill msd0\n", iMPI);
                                }
                            }                        
                        }
            ...
            if( bRemoveDriftOfMSD ){
                ...
                // subtract average to get no drift trajectories
                #pragma omp parallel for
                for(int i=0;i<trajectoryList.size();i++){
                    for(int j=0;j<trajectoryList[i].size();j++){
                        trajectoryList[i][j] -= trajectoryAverage[j];
                    ...
                    MeanSquareDisplacementBruteForce(
                        &trajectoryList[i], &msd[i], &interval);
                }
            }
